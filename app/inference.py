...
while cap.isOpened():
    ...
    if recording and video_writer is None:
        ...
        video_writer = cv2.VideoWriter(OUTPUT_FILEPATH, fourcc, fps, (w, h))
    ...
    if results.hand_landmarks:
        ...
        for connection in CONNECTIONS:
            cv2.line(frame,
                     pixel_coords[connection[0]],
                     pixel_coords[connection[1]],
                     (0,255,255), 2)
    ...
    cv2.putText(frame, f"Gesture: {label_text}",
                (22, 50), font, 0.75, (0, 255, 0), 2)

    cv2.putText(frame, f"Confidence: {confidence_value*100:.1f}%",
                (22, 85), font, 0.65, (0, 255, 0), 2)

    draw_progress_bar(frame, 22, 105, 260, 14, confidence_value)

    # Glass panel for ESC hint (bottom-right)
    panel_width = 240
    panel_height = 50
